        ...
        <transition name="fade" appear>
            <div v-if="!loading" class="flex flex-col gap-y-4 pb-4">
                ...
                <div class="px-4 flex justify-between items-start gap-x-2">
                    <h1 class="font-medium text-lg">{{ product.title }} {{ activeTab.subscribe?.title ? activeTab.subscribe.title : '' }} {{ activeTab.plan.title }}</h1>
                ...
                <div v-if="product.template === 'PP'" class="flex flex-col gap-y-4">
                    ...
                    <div class="px-4">
                        <div class="flex w-full justify-between border-[1.5px] border-hint_color rounded-xl p-0.5">
                            <button @click="() => { activeTab.subscribe = el }" v-for="el, index in plus" :key="index" :class="['flex font-medium text-sm p-2.5 transition-all rounded-lg w-1/3 justify-center', activeTab.subscribe?.id === el.id && 'from-[#4F7DF9] to-[#2E60E7] bg-gradient-to-b']">{{ el.title }}</button>
                        </div>
                    </div>
                ...
                <div v-if="product.template === 'GP'" class="flex flex-col gap-y-4">
                    ...
                    <button @click="() => {
                        ...
                        <span class="w-8 h-8 rounded-lg bg-white shadow-sm  overflow-clip">
                            <span v-if="activeTab.subscribe === 'For new users'" class="flex w-full h-full justify-center items-center from-[#6BF792] to-[#36A254] bg-gradient-to-b rounded-lg">
